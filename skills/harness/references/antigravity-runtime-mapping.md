...
| 스킬 카탈로그 | `cursor-skills` (GitHub) | **`E:\workspace\skills_안티그래비티\antigravity`** (로컬, `HARNESS_SKILL_CATALOG`) |

## 스킬 카탈로그 (필수)

추가·공유 스킬은 **반드시** 아래 경로에서 선별한다:

```powershell
$env:HARNESS_SKILL_CATALOG = "E:\workspace\skills_안티그래비티\antigravity"
```

`install-skills.ps1` / `provision-skill.ps1`은 이 경로를 기본 카탈로그로 사용한다. 카탈로그 전체를 `~/.gemini/antigravity/skills/`에 복사하지 말 것 — lock에 있는 ID만 Junction/Copy.
