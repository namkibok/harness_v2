...
```
[리더/오케스트레이터]
    ├── 병렬 Task(research-team)
    ├── TodoWrite(4개 조사 작업)
    ├── 팀원들이 자체 조율 (handoff 파일 또는 다음 Task prompt에 맥락 포함)
    ├── 결과 수집 (Read)
    └── 종합 보고서 생성
```
...
Phase 2: 팀 구성
  - 병렬 Task(team_name: "research-team", members: [
...
Phase 3: 조사 수행
  - 4명의 팀원이 독립적으로 조사
  - 흥미로운 발견이 있으면 팀원 간 handoff 파일 또는 다음 Task prompt에 맥락 포함로 공유
...
```
official ──handoff 파일 또는 다음 Task prompt에 맥락 포함──→ background  (관련 공식 발표 공유)
media ────handoff 파일 또는 다음 Task prompt에 맥락 포함──→ background  (투자/인수 정보 공유)
community ─handoff 파일 또는 다음 Task prompt에 맥락 포함──→ media      (커뮤니티 반응 중 미디어 관련 정보)
모든 팀원 ──TodoWrite──→ 공유 작업 목록  (진행률 업데이트)
리더 ←───── 유휴 알림 ──── 완료된 팀원   (자동)
```

---

## 예시 2: SF 소설 집필 팀 (병렬 Task 팀 모드)

### 팀 아키텍처: 파이프라인 + 팬아웃
### 실행 모드: 병렬 Task 팀

```
Phase 1 (병렬 — 병렬 Task 팀): worldbuilder + character-designer + plot-architect
  → 서로 handoff 파일 또는 다음 Task prompt에 맥락 포함로 일관성 조율
Phase 2 (순차): prose-stylist (집필)
Phase 3 (병렬 — 병렬 Task 팀): science-consultant + continuity-manager (리뷰)
  → 서로 handoff 파일 또는 다음 Task prompt에 맥락 포함로 발견 공유
...
## 팀 통신 프로토콜
- character-designer에게: 사회 구조, 계급 시스템, 직업군 정보 handoff 파일 또는 다음 Task prompt에 맥락 포함
- plot-architect에게: 세계의 주요 갈등 구조, 위기 요소 handoff 파일 또는 다음 Task prompt에 맥락 포함
...
```
Phase 1: 병렬 Task(team_name: "novel-team", members: [worldbuilder, character-designer, plot-architect])
         TodoWrite([세계관 구축, 캐릭터 설계, 플롯 구조])
         → 팀원들이 자체 조율하며 병렬 작업
         → worldbuilder가 사회 구조 완성 시 character-designer에게 handoff 파일 또는 다음 Task prompt에 맥락 포함
         → character-designer가 주인공 설정 시 plot-architect에게 handoff 파일 또는 다음 Task prompt에 맥락 포함
...
Phase 3: 새 팀 생성 — 병렬 Task(team_name: "review-team", members: [science-consultant, continuity-manager])
...
## 예시 4: 코드 리뷰 팀 (병렬 Task 팀 모드)

### 팀 아키텍처: 팬아웃/팬인 + 토론
### 실행 모드: 병렬 Task 팀

> 코드 리뷰는 병렬 Task 팀이 빛나는 대표적 사례. 서로 다른 관점의 리뷰어들이 발견을 공유하고 도전하면서 더 깊은 리뷰가 가능.

```
[리더] → 병렬 Task(review-team)
    ├── security-reviewer: 보안 취약점 점검
    ├── performance-reviewer: 성능 영향 분석
    └── test-reviewer: 테스트 커버리지 검증
    → 리뷰어들이 서로 발견 공유 (handoff 파일 또는 다음 Task prompt에 맥락 포함)
    → 리더가 결과 종합
```
...
```
security ──handoff 파일 또는 다음 Task prompt에 맥락 포함──→ performance  ("이 SQL 쿼리 주입 가능, 성능 측면에서도 확인 필요")
performance ──handoff 파일 또는 다음 Task prompt에 맥락 포함──→ test      ("N+1 쿼리 발견, 관련 테스트 있는지 확인 부탁")
test ────handoff 파일 또는 다음 Task prompt에 맥락 포함──→ security      ("인증 모듈 테스트 없음, 보안 관점에서 우선순위 의견?")
```
...
## 예시 5: 감독자 패턴 — 코드 마이그레이션 팀 (병렬 Task 팀 모드)
...
### 감독자의 동적 분배 로직 (병렬 Task 팀 활용)

```
1. 전체 대상 파일 목록 수집
2. 복잡도 추정 (파일 크기, import 수, 의존성)
3. TodoWrite로 파일 배치를 작업으로 등록 (의존성 포함)
4. 팀원들이 자체적으로 작업 요청 (claim)
5. 팀원이 TodoWrite로 완료 보고 시:
   - 성공 → 다음 작업 자동 요청
   - 실패 → 리더가 handoff 파일 또는 다음 Task prompt에 맥락 포함로 원인 확인 → 재할당 또는 다른 팀원에게 배정
...
**실행 모드를 반드시 명시** — 병렬 Task 팀(기본) 또는 서브 에이전트.
