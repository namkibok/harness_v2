...
3. 채택 스킬은 프로젝트 `.cursor/skills/{id}`에 **정션 링크 또는 복사** — `~/.cursor/skills/`에 카탈로그 전체 복사 금지
4. 없거나 프로젝트 전용이면 신규 `SKILL.md` 작성
5. 오케스트레이터·에이전트 정의에 `Read` 경로 명시
...
- [ ] `프로젝트/.cursor/skills/` — 스킬 파일들 (SKILL.md + references/)
