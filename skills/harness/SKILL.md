...
도메인/프로젝트에 맞는 하네스를 구성하고, 각 에이전트의 역할을 정의하며, 에이전트가 사용할 스킬을 생성하는 **Cursor용** 메타 스킬.

> Claude Code 원본 대비 런타임 차이: `references/cursor-runtime-mapping.md`

**핵심 원칙:**
1. 에이전트 정의(`.cursor/agents/`)와 스킬(`.cursor/skills/`)을 생성한다.
2. **병렬 Task 팀**을 기본 실행 모드로 사용한다 (2명 이상 협업 시 `Task` + `run_in_background`).
3. **AGENTS.md**(또는 `.cursor/rules/harness.mdc`)에 하네스 포인터를 등록한다 — 트리거 규칙 + 변경 이력만.
...
1. `프로젝트/.cursor/agents/`, `프로젝트/.cursor/skills/`, `프로젝트/AGENTS.md`를 읽는다
...
**병렬 Task 팀이 최우선 기본값이다.** 2개 이상 협업 시 `Task`를 `run_in_background: true`로 병렬 실행하고, `_workspace/` 파일·handoff 메모로 조율한다. (Claude Code의 Agent Teams에 해당하는 Cursor 패턴.)
...
**모든 에이전트는 반드시 `프로젝트/.cursor/agents/{name}.md` 파일로 정의한다.** 에이전트 정의 파일 없이 Task 도구의 prompt에 역할을 직접 넣는 것은 금지한다. 이유:
- 에이전트 정의가 파일로 존재해야 다음 세션에서 재사용 가능
- 팀 통신 프로토콜이 명시되어야 에이전트 간 협업 품질 보장
- 하네스의 핵심 가치는 에이전트(누가)와 스킬(어떻게)의 분리

빌트인 타입(`generalPurpose`, `explore`, `shell`)을 쓰더라도 `.cursor/agents/{name}.md`는 **반드시** 생성한다. `subagent_type`은 빌트인 이름 또는 **커스텀 에이전트 파일의 `name`**(예: `code-reviewer`)을 사용한다.

**모델:** 사용자가 모델을 지정했을 때만 Task의 `model` 파라미터를 넣는다. 미지정 시 Cursor 기본 모델을 따른다.
...
각 에이전트가 사용할 스킬을 `프로젝트/.cursor/skills/{name}/SKILL.md`에 연결하거나 생성한다. 상세 작성 가이드는 `references/skill-writing-guide.md` 참조.

#### 4-0. 공유 카탈로그에서 선별 (팀 Git 연동)

**카탈로그 경로** (우선순위): `HARNESS_SKILL_CATALOG` → harness_v2의 `skills/catalog/` → 프로젝트 `skills/catalog/`.  
절차·정션 링크·금지 사항: `references/skill-catalog.md`. 도메인별 추천 ID: harness_v2 루트의 `skills/catalog-index.yaml`.

1. 도메인에 맞는 스킬 ID를 `catalog-index.yaml`과 카탈로그 `Glob`으로 후보화 (보통 3~8개)
2. `{catalog}/{id}/SKILL.md`를 Read해 에이전트별로 채택/제외
3. 채택 스킬은 `scripts/install-skills.ps1`로 프로젝트 `.cursor/skills/`에 설치 (sparse fetch, Junction 기본) — `~/.cursor/skills/`에 카탈로그 전체 복사 금지
...
| **파일 기반** | `_workspace/{phase}_{agent}_*.md` | 공통 | **Cursor에서 기본·권장** |
...
**AGENTS.md에 넣지 않는 것:** 에이전트 목록, 스킬 목록, 디렉토리 구조, 실행 규칙 상세. 이유: 에이전트/스킬 목록은 오케스트레이터 스킬과 `.cursor/agents/`, `.cursor/skills/`에서 관리하므로 중복이다. 디렉토리 구조는 파일 시스템에서 직접 확인 가능하다. AGENTS.md는 **포인터(트리거 규칙) + 변경 이력**만 담는다.
...
**Step 1: 현황 감사**
- `.cursor/agents/` 파일 목록과 오케스트레이터 스킬의 에이전트 구성 비교 → 불일치 목록 생성
- `.cursor/skills/` 디렉토리 목록과 오케스트레이터 스킬의 스킬 구성 비교 → 불일치 목록 생성
- 감사 결과를 사용자에게 보고한다
...
- [ ] `프로젝트/.cursor/agents/` — **에이전트 정의 파일 필수 생성** (빌트인 타입이라도 파일 생성 필수)
- [ ] **`.harness/skills.lock.yaml`** — 카탈로그에서 채택한 스킬 ID 목록 (팀 repo 커밋)
- [ ] **`install-skills.ps1` 실행** 또는 `.cursor/skills/` 커밋으로 팀 온보딩 경로 확보
...
- **Cursor 런타임 매핑**: `references/cursor-runtime-mapping.md`
